  ...
  const cookiesFormatted = req.cookies ? Object.keys(req.cookies).map(key => `${key}: ${req.cookies[key]}`).join('\n') : "";
        ...
        const html = `
            ...
            <body>
                ...
                <p><strong>Server Time (UTC):</strong> <span class="highlight">${serverTime}</span></p>
                <p><strong>Requested:</strong> <span class="highlight">${req.protocol}://${req.get('host')}${req.originalUrl}</span></p>
                <p><strong>Your IP:</strong> <span class="highlight">${clientIp}</span></p>
                <p><strong>Remote Port:</strong> <span class="highlight">${remotePort}</span></p>
                <p><strong>User-Agent:</strong> <span class="highlight">${userAgent}</span></p>
                <h2>Headers:</h2>
                <table>
                    <thead>
                        <tr>
